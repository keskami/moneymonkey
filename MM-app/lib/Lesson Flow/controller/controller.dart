  ...
  void loadQuestion() {
    ...
    if (currentQuestionIndex.value < quizQuestions.length) {
      ...
      correctAnswer.value = currentQuestion['correctAnswer'] ??
          ...
          '';
      quizOptions.value = List<String>.from(
          currentQuestion['option'].values.map((e) => e.toString()));
...
// Fetch flashcards from Firestore based on the current lesson
  Future<void> fetchFlashcards(String lessonId) async {
    try {
      String lessonId =
          'lesson${currentLessonIndex.value + 1}'; // Generate the lesson ID
      print(lessonId);
      final lessonDoc = await FirebaseFirestore.instance
          .collection('lessons')
          .doc(lessonId)
      ...
      if (lessonDoc.exists) {
        ...
        final flashcardsData =
            lessonDoc.data()?['flashcards'] as List<dynamic>?;
