...
class BaseLessonController extends GetxController {
  ...
  Future<void> loadConceptData() async {
    ...
    } else if (type == ComponentType.scenarioSimulation) {
      try {
        List<Widget> mcqPages = [];
        final Component data =
            await localAcademicService.getComponent(componentId);
        for (int i = 0; i < data.questionData.length; i++) {
          if (data.questionData[i].type == SubComponentType.scenarioquestion) {
            for (int k = 0; k < data.questionData[i].data.length; k++) {
              ScenarioQuestion scenarioQuestion = data.questionData[i].data[k];
              String title1 = scenarioQuestion.options[0].title;
              String title2 = scenarioQuestion.options[1].title;
              String title3 = scenarioQuestion.options[2].title;
              mcqPages.add(MonkeyMCQPage(
                  question: scenarioQuestion.questionText,
                  options: [
                    [title1, scenarioQuestion.options[0].iconUrl],
                    [title2, scenarioQuestion.options[1].iconUrl],
                    [title3, scenarioQuestion.options[2].iconUrl]
                  ],
                  correctMessages: {
                    title1: scenarioQuestion.feedback[title1] ?? "",
                    title2: scenarioQuestion.feedback[title2] ?? "",
                    title3: scenarioQuestion.feedback[title3] ?? ""
                  },
                  scores: {
                    title1: scenarioQuestion.options[0].score,
                    title2: scenarioQuestion.options[1].score,
                    title3: scenarioQuestion.options[2].score
                  }));
            }
          }
        }
        ...
        pages.add(
          TapToRevealPictorialPage(
            title: data.questionData[0].data.scenario,
            button: "Start Managing Your Money",
            items: List<String>.from(
              data.questionData[0].data.options.map((item) => item.title),
            ),
            itemImgs: List<String>.from(
              data.questionData[0].data.options.map((item) => item.iconUrl),
            ),
            monkeyImg: data.questionData[0].data.mintyImage,
            instructions: [
              "Click for choice 1...",
              "Click for choice 2...",
              "Click for choice 3..."
